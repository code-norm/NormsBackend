...
func SignupHandler(db *sql.DB, u *cL.User) http.HandlerFunc {
	fn := func(w http.ResponseWriter, r *http.Request) {
		...
		*u = newUser
		...
		_, err = db.Query("CREATE TABLE " + surv + " (gluten VARCHAR(60), sugar VARCHAR(60), satfat VARCHAR(60), alchohol VARCHAR(60), refgrains VARCHAR(60), msg VARCHAR(60), salt VARCHAR(60)")
		if err != nil {
			// If there is any issue with inserting into the database, return a 500 error
			fmt.Println(err)
			return
